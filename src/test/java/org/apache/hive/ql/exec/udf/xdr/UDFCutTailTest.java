...
public class UDFCutTailTest extends UDFBaseTest {

	@Override
	public GenericUDF createUDF() {
		return new UDFCutTail();
	}

	@Override
	public ObjectInspector[] argumentsOIs() {
		return new ObjectInspector[] {
				javaStringObjectInspector, writableIntObjectInspector
		};
	}

	@Test
	public void testUdf_normalTextValue_ok() throws HiveException {
		Text actual = evaluate("12345678910", 4);
	...
	public void testUdf_normalBinaryValue_ok() throws HiveException {
		udf.initialize(new ObjectInspector[] {
				ImmutableOI, writableIntObjectInspector });

		final byte[] expected = "1234567".getBytes();
		ImmutableBytesWritable actual = evaluate(
				new ImmutableBytesWritable("12345678910".getBytes(), true), 4);
	...
	public void testUdf_abnormalTextValue_ok() throws HiveException {
		Text actual = evaluate(null, 7);
		assertNull(actual);

		final String value = "19999999999";

		actual = evaluate(value, 10000);
		assertNull(actual);

		actual = evaluate(value, -1);
		assertNull(actual);
