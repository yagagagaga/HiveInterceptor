/*
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.apache.util;

import java.util.Arrays;

public class SM4New {
	private static final int ENCRYPT = 1;
	private static final int DECRYPT = 0;
	private static final int BLOCK = 16;
	private final byte[] lowerHex = {
			0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39,
			0x61, 0x62, 0x63, 0x64, 0x65, 0x66
	};
	private final int[] round_key = new int[32];
	private final byte[] inputRound = new byte[128];
	private final byte[] outputRound = new byte[128];
	private final int[] CK = { 0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
			0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9, 0xe0e7eef5,
			0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81,
			0x888f969d, 0xa4abb2b9, 0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d,
			0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
			0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25,
			0x2c333a41, 0x484f565d, 0x646b7279 };
	int[] Sbox = {  };
	private byte[] lastkey;

	protected static int toDigit(byte ch) {
		return Character.digit(ch, 16);
	}

	private int rotl(int x, int y) {
		return x << y | x >>> (32 - y);
	}

	private int byteSub(int A) {
		return (Sbox[A >>> 24]) << 24
				| (Sbox[A >> 16 & 0xFF]) << 16
				| (Sbox[A >> 8 & 0xFF]) << 8 | (Sbox[A & 0xFF]);
	}

	private int l1(int B) {
		return B ^ rotl(B, 2) ^ rotl(B, 10) ^ rotl(B, 18) ^ rotl(B, 24);
	}

	private int l2(int B) {
		return B ^ rotl(B, 13) ^ rotl(B, 23);
	}

	void sms4Crypt(byte[] Input, byte[] Output, int offset, int[] rk) {
		int mid, x0, x1, x2, x3;

		x0 = (Input[offset] << 24) | ((Input[offset + 1] & 0xff) << 16) |
				((Input[offset + 2] & 0xff) << 8) | ((Input[offset + 3] & 0xff));
		x1 = (Input[offset + 4] << 24) | ((Input[offset + 5] & 0xff) << 16) |
				((Input[offset + 6] & 0xff) << 8) | ((Input[offset + 7] & 0xff));
		x2 = (Input[offset + 8] << 24) | ((Input[offset + 9] & 0xff) << 16) |
				((Input[offset + 10] & 0xff) << 8) | ((Input[offset + 11] & 0xff));
		x3 = (Input[offset + 12] << 24) | ((Input[offset + 13] & 0xff) << 16) |
				((Input[offset + 14] & 0xff) << 8) | ((Input[offset + 15] & 0xff));

		mid = x1 ^ x2 ^ x3 ^ rk[0];
		mid = byteSub(mid);
		x0 ^= l1(mid); // x4

		mid = x2 ^ x3 ^ x0 ^ rk[1];
		mid = byteSub(mid);
		x1 ^= l1(mid); // x5

		mid = x3 ^ x0 ^ x1 ^ rk[2];
		mid = byteSub(mid);
		x2 ^= l1(mid); // x6

		mid = x0 ^ x1 ^ x2 ^ rk[3];
		mid = byteSub(mid);
		x3 ^= l1(mid); // x7

		mid = x1 ^ x2 ^ x3 ^ rk[4];
		mid = byteSub(mid);
		x0 ^= l1(mid); // x4

		mid = x2 ^ x3 ^ x0 ^ rk[5];
		mid = byteSub(mid);
		x1 ^= l1(mid); // x5

		mid = x3 ^ x0 ^ x1 ^ rk[6];
		mid = byteSub(mid);
		x2 ^= l1(mid); // x6

		mid = x0 ^ x1 ^ x2 ^ rk[7];
		mid = byteSub(mid);
		x3 ^= l1(mid); // x7

		mid = x1 ^ x2 ^ x3 ^ rk[8];
		mid = byteSub(mid);
		x0 ^= l1(mid); // x4

		mid = x2 ^ x3 ^ x0 ^ rk[9];
		mid = byteSub(mid);
		x1 ^= l1(mid); // x5

		mid = x3 ^ x0 ^ x1 ^ rk[10];
		mid = byteSub(mid);
		x2 ^= l1(mid); // x6

		mid = x0 ^ x1 ^ x2 ^ rk[11];
		mid = byteSub(mid);
		x3 ^= l1(mid); // x7

		mid = x1 ^ x2 ^ x3 ^ rk[12];
		mid = byteSub(mid);
		x0 ^= l1(mid); // x4

		mid = x2 ^ x3 ^ x0 ^ rk[13];
		mid = byteSub(mid);
		x1 ^= l1(mid); // x5

		mid = x3 ^ x0 ^ x1 ^ rk[14];
		mid = byteSub(mid);
		x2 ^= l1(mid); // x6

		mid = x0 ^ x1 ^ x2 ^ rk[15];
		mid = byteSub(mid);
		x3 ^= l1(mid); // x7

		mid = x1 ^ x2 ^ x3 ^ rk[16];
		mid = byteSub(mid);
		x0 ^= l1(mid); // x4

		mid = x2 ^ x3 ^ x0 ^ rk[17];
		mid = byteSub(mid);
		x1 ^= l1(mid); // x5

		mid = x3 ^ x0 ^ x1 ^ rk[18];
		mid = byteSub(mid);
		x2 ^= l1(mid); // x6

		mid = x0 ^ x1 ^ x2 ^ rk[19];
		mid = byteSub(mid);
		x3 ^= l1(mid); // x7

		mid = x1 ^ x2 ^ x3 ^ rk[20];
		mid = byteSub(mid);
		x0 ^= l1(mid); // x4

		mid = x2 ^ x3 ^ x0 ^ rk[21];
		mid = byteSub(mid);
		x1 ^= l1(mid); // x5

		mid = x3 ^ x0 ^ x1 ^ rk[22];
		mid = byteSub(mid);
		x2 ^= l1(mid); // x6

		mid = x0 ^ x1 ^ x2 ^ rk[23];
		mid = byteSub(mid);
		x3 ^= l1(mid); // x7

		mid = x1 ^ x2 ^ x3 ^ rk[24];
		mid = byteSub(mid);
		x0 ^= l1(mid); // x4

		mid = x2 ^ x3 ^ x0 ^ rk[25];
		mid = byteSub(mid);
		x1 ^= l1(mid); // x5

		mid = x3 ^ x0 ^ x1 ^ rk[26];
		mid = byteSub(mid);
		x2 ^= l1(mid); // x6

		mid = x0 ^ x1 ^ x2 ^ rk[27];
		mid = byteSub(mid);
		x3 ^= l1(mid); // x7

		mid = x1 ^ x2 ^ x3 ^ rk[28];
		mid = byteSub(mid);
		x0 ^= l1(mid); // x4

		mid = x2 ^ x3 ^ x0 ^ rk[29];
		mid = byteSub(mid);
		x1 ^= l1(mid); // x5

		mid = x3 ^ x0 ^ x1 ^ rk[30];
		mid = byteSub(mid);
		x2 ^= l1(mid); // x6

		mid = x0 ^ x1 ^ x2 ^ rk[31];
		mid = byteSub(mid);
		x3 ^= l1(mid); // x7

//    for (r = 0; r < 32; r += 4) {
//      mid = x1 ^ x2 ^ x3 ^ rk[r + 0];
//      mid = ByteSub(mid);
//      x0 ^= L1(mid); // x4
//
//      mid = x2 ^ x3 ^ x0 ^ rk[r + 1];
//      mid = ByteSub(mid);
//      x1 ^= L1(mid); // x5
//
//      mid = x3 ^ x0 ^ x1 ^ rk[r + 2];
//      mid = ByteSub(mid);
//      x2 ^= L1(mid); // x6
//
//      mid = x0 ^ x1 ^ x2 ^ rk[r + 3];
//      mid = ByteSub(mid);
//      x3 ^= L1(mid); // x7
//    }

		Output[offset] = (byte) (x3 >> 24);
		Output[offset + 1] = (byte) (x3 >> 16);
		Output[offset + 2] = (byte) (x3 >> 8);
		Output[offset + 3] = (byte) x3;

		Output[offset + 4] = (byte) (x2 >> 24);
		Output[offset + 5] = (byte) (x2 >> 16);
		Output[offset + 6] = (byte) (x2 >> 8);
		Output[offset + 7] = (byte) x2;

		Output[offset + 8] = (byte) (x1 >> 24);
		Output[offset + 9] = (byte) (x1 >> 16);
		Output[offset + 10] = (byte) (x1 >> 8);
		Output[offset + 11] = (byte) x1;

		Output[offset + 12] = (byte) (x0 >> 24);
		Output[offset + 13] = (byte) (x0 >> 16);
		Output[offset + 14] = (byte) (x0 >> 8);
		Output[offset + 15] = (byte) x0;
	}

	private void sms4Keyext(byte[] Key, int[] rk, int CryptFlag) {
		int r, mid;
		int x0, x1, x2, x3;

		x0 = Key[0] << 24;
		x0 |= Key[1] << 16;
		x0 |= Key[2] << 8;
		x0 |= Key[3];

		x1 = Key[4] << 24;
		x1 |= Key[5] << 16;
		x1 |= Key[6] << 8;
		x1 |= Key[7];

		x2 = Key[8] << 24;
		x2 |= Key[9] << 16;
		x2 |= Key[10] << 8;
		x2 |= Key[11];

		x3 = Key[12] << 24;
		x3 |= Key[13] << 16;
		x3 |= Key[14] << 8;
		x3 |= Key[15];

		x0 ^= 0xa3b1bac6;
		x1 ^= 0x56aa3350;
		x2 ^= 0x677d9197;
		x3 ^= 0xb27022dc;

		for (r = 0; r < 32; r += 4) {
			mid = x1 ^ x2 ^ x3 ^ CK[r];
			mid = byteSub(mid);
			rk[r] = x0 ^= l2(mid); // rk0=K4

			mid = x2 ^ x3 ^ x0 ^ CK[r + 1];
			mid = byteSub(mid);
			rk[r + 1] = x1 ^= l2(mid); // rk1=K5

			mid = x3 ^ x0 ^ x1 ^ CK[r + 2];
			mid = byteSub(mid);
			rk[r + 2] = x2 ^= l2(mid); // rk2=K6

			mid = x0 ^ x1 ^ x2 ^ CK[r + 3];
			mid = byteSub(mid);
			rk[r + 3] = x3 ^= l2(mid); // rk3=K7
		}

		// 解密时轮密钥使用顺序：rk31,rk30,...,rk0
		if (CryptFlag == DECRYPT) {
			for (r = 0; r < 16; r++) {
				mid = rk[r];
				rk[r] = rk[31 - r];
				rk[31 - r] = mid;
			}
		}
	}

	public int encrypt(byte[] in, int offset, int inLen, byte[] key, byte[] output) {
		int roundLen = ((inLen & 0x0f) > 0) ? ((inLen & 0xfff0) + 16) : inLen;
		System.arraycopy(in, offset, inputRound, 0, inLen);
		Arrays.fill(inputRound, inLen, roundLen, (byte) 32);
		sms4(inputRound, roundLen, key, outputRound, ENCRYPT);
		return encodeHex(outputRound, roundLen, output);
	}

	private int encodeHex(byte[] in, int length, byte[] output) {
		int i = 0;
		int j = 0;
		for (; i < length; ++i) {
			output[j++] = lowerHex[(in[i] >> 4) & 0x0f];
			output[j++] = lowerHex[15 & in[i]];
		}

		return j;
	}

	private byte[] decodeHex(byte[] in, int length) {

		byte[] out = new byte[length >> 1];
		int i = 0;

		for (int j = 0; j < length; ++i) {
			int f = toDigit(in[j]) << 4;
			++j;
			f |= toDigit(in[j]);
			++j;
			out[i] = (byte) (f & 255);
		}

		return out;
	}

	public byte[] decrypt(byte[] in, int inLen, byte[] key) {
		byte[] input = decodeHex(in, inLen);
		byte[] output = new byte[input.length];
		sms4(input, input.length, key, output, DECRYPT);

		int i = input.length - 1;
		for (; i >= 0; --i) {
			if (output[i] != 32) {
				break;
			}
		}

		return Arrays.copyOf(output, i + 1);
	}

	public int sms4(byte[] in, int inLen, byte[] key, byte[] out, int CryptFlag) {
		int point = 0;

		if (!Arrays.equals(lastkey, key)) {
			sms4Keyext(key, round_key, CryptFlag);
			lastkey = key;
		}

		while (inLen >= BLOCK) {
			sms4Crypt(in, out, point, round_key);
			inLen -= BLOCK;
			point += BLOCK;
		}

		return 0;
	}
}