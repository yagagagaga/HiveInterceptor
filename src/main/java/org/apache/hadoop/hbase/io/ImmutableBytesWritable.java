...
/**
 * 本代码抄自 {@link org.apache.hadoop.hbase.io.ImmutableBytesWritable}，唯一不同是：
 * 增加了一个字段：{@link #isChars}
 * <p>
 ...
 * comparatively 'immutable'. When creating a new instance of this class, the
 * underlying byte [] is not copied, just referenced. The backing buffer is
 * accessed when we go to serialize.
 */
public class ImmutableBytesWritable
		implements WritableComparable<ImmutableBytesWritable> {
	static { // register this comparator
		WritableComparator
				.define(ImmutableBytesWritable.class, new Comparator());
	}
	 ...
	 */
	public ImmutableBytesWritable(boolean isChars) {
		super();
	 ...
	 */
	public ImmutableBytesWritable(byte[] bytes, boolean isChars) {
		this(bytes, 0, bytes.length, isChars);
	}
	...
	public ImmutableBytesWritable(final byte[] bytes, final int offset,
			final int length, boolean isChars) {
		this.bytes = bytes;
		this.offset = offset;
		this.length = length;
		this.isChars = isChars;
